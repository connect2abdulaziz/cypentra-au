...
import { getProfile } from '@/services/reviews'
...
const Reviews = () => {
  ...
  if (!profileData || !profileData.rating_distribution) return null

  const { total_reviews, rating_distribution, trust_score } = profileData

  // Get Trustpilot URL
  const trustpilotUrl =
    profileData.trustpilot_url ||
    profileData.url ||
    profileData.website_url ||
    `https://www.trustpilot.com/review/${profileData.domain || profileData.business_unit_id || 'cypentra.com'}`
  ...
  const calculateAverageRating = () => {
    if (!rating_distribution || typeof rating_distribution !== 'object') {
      return 5.0
    }
  ...
  const renderTrustpilotStars = (rating = 5) => {
    const stars = []
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= fullStars) {
        // Full star
        stars.push(
          <div key={i} className='bg-[#0091a4] p-1'>
            <svg
              width='16'
              height='16'
              viewBox='0 0 24 24'
        ...
        stars.push(
          <div key={i} className='bg-gray-200 p-1 relative overflow-hidden'>
            ...
            {/* Filled portion */}
            <div
              className='absolute inset-0 bg-[#0091a4] p-1'
    ...
    <section className='w-full py-8 sm:py-12 md:py-16 lg:py-20 grid-background'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-16'>
        <div
          ...
          className='flex flex-col lg:flex-row gap-6 lg:gap-8 bg-white w-full rounded-lg cursor-pointer hover:shadow-xl transition-all duration-300 hover:scale-[1.01]'
        >
          ...
          <div className='flex-1 p-4 sm:p-6 lg:p-8'>
            <div className='flex flex-col sm:flex-row items-start gap-4'>
              <div className='flex-1 w-full'>
                ...
                {/* Service Type */}
                <span className='text-[#0091a4] text-sm sm:text-base'>
                  ...
                  return (
                    <div key={stars} className='flex items-center gap-2'>
                      ...
                      <div className='flex-1 bg-gray-200 rounded-full h-2.5 sm:h-3 relative'>
                        <div
                          className='bg-[#0091a4] h-full rounded-full transition-all duration-300'
      ...
      {/* Reviews Carousel */}
      <ReviewsCarousel trustpilotUrl={trustpilotUrl} />
    </section>
  )
}
